...
string valueOf(string name, bool useDummyValues = false)
{
	...
	{
		if (find(csect->extRef.begin(), csect->extRef.end(), name) != csect->extRef.end())
		{
			return "0";
		}
		else
...
string substitute(string expression, bool useDummyValues = false)
{
	...
	for (unsigned int i = 0; i < expression.length();)
	{
		...
		if (name.compare("") != 0)
			exp += valueOf(name, useDummyValues);
					   ...
					   bool *retFlagIsRelative)
{
	...
	{
		op = "";
		while ((i < exp.length() && !isOperator(exp[i])) || (exp[i] == '-' && op.length() == 0 && !lastAddedIsOperand))
		{
			op += exp[i++];
		}
		if (op.compare("-") == 0)
		...
		else if (operations.empty() || exp[i] == '(' || comparePeriority(operations.top(), exp[i]) != 1)
		{
