...
bool hasExternalReference(parsedLine pl, cSect *current, vector<string> *refs,
						  vector<char> *signs)
{
	if (pl.operand1.find("(") != string::npos && pl.operand1.find(")") != string::npos)
	{
	...
	{
		if (find(current->extRef.begin(), current->extRef.end(), all[i]) != current->extRef.end())
		{
...
{
	if (pl.lable.compare("") == 0 && pl.opcode.compare("") == 0 && pl.operand1.compare("") == 0 && pl.operand2.compare("") == 0)
		return true;
...
cSect runPass1(vector<parsedLine> v, bool *started, bool *ended,
			   bool *successfull, vector<string> _names, int idx)
{
	parsedLine pl;
	cout << pl.opcode << " " << pl.lable << endl;
	cSect current;
