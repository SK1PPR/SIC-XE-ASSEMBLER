...
int main2()
{
	...
	for (map<string, symInfo *>::iterator it = myMap->begin(); it != myMap->end(); ++it)
	{
						   ...
						   string operands, bitset<6> flags, string opCode)
{
	...
	line = lineNumberStr + addressStr + label + op_str + operands + mod_flags_str + opCode;
	return line;
}
...
void writeSymTab(ofstream *file, map<string, symInfo *> *theMap)
{
	...
	temp += "\t|";
	writeLine(file, temp);
	for (map<string, symInfo *>::const_iterator it = theMap->begin(); it != theMap->end(); it++)
	{
...
{ // 1
	...
	string line = "H^";
	if (prog_name.compare("") != 6)
		adjustStringLength(&prog_name, 6, spaceChar, false);
	line += prog_name;
	line += "^";
...
{ // 4
	...
	for (unsigned int i = 0; i < opCodes->size();)
	{
		...
		while (1)
		{
			...
			temp += (*opCodes)[i];
			if (i + 1 < (*opCodes).size())
				end_address = (*lpl)[i + 1].loc;
			else
				end_address = (*lpl)[i].loc;
			i++;
			x++;
		}
...
{ // 2
	...
	for (; (unsigned)i < exdefs->size(); i++)
	{
		...
		do
		{
			...
			line += "^";
			line += address_str;
			if ((i + 1) % 5 != 0 && (unsigned)(i + 1) < exdefs->size())
				line += "^";
			i++;
		} while (i % 5 != 0);
		if ((unsigned)i >= exdefs->size())
...
{ // 3
	...
	for (; (unsigned)i < exrefs->size(); i++)
	{
		...
		do
		{
			if ((unsigned)i >= exrefs->size())
				break;
			line += (*exrefs)[i];
			if ((i + 1) % 5 != 0 && (unsigned)(i + 1) < exrefs->size())
				line += "^";
			i++;
		} while (i % 5 != 0);
		if ((unsigned)i >= exrefs->size())
