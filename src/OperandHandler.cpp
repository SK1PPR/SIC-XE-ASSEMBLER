...
int getValueOf(string name)
{
	...
	{
		if (find(curCSECT->extRef.begin(), curCSECT->extRef.end(), name) != curCSECT->extRef.end())
		{
			return 0;
		}
		else
...
{
	if (op.length() < 3 || op[0] != 'X' || op[1] != '\'' || op[op.length() - 1] != '\'')
		return false;
...
{
	if (op.length() < 3 || op[0] != 'C' || op[1] != '\'' || op[op.length() - 1] != '\'')
		return false;
	return true;
}
...
bool isValidImmediateOp(string op)
{
	...
	return isValidNum(rest) || isValidOpNm(rest) || isValidHexConstant(rest) || isValidByteConstant(rest) || isValidAlphaNumeric(rest);
}
...
bool isValidLiteralOp(string op)
{
	if (op.length() < 2 || op[0] != '=')
		return false;
	string rest = op.substr(1);
	return isValidNum(rest) || isValidOpNm(rest) || isValidHexConstant(rest) || isValidByteConstant(rest);
}
