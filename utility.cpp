...
int stringHexToInt(string x) { return stoul(x, nullptr, 16); }
...
void readFirstNonWhiteSpace(string line, int &index, bool &status, string &data,
  ...
  while (index < line.length() &&
         !checkWhiteSpace(line[index])) { // If no whitespace then data
    data += line[index];
    index++;
  }
  ...
  while (
      index < line.length() &&
      checkWhiteSpace(line[index])) { // Increase index to pass all whitespace
    index++;
  }
}
...
void readByteOperand(string line, int &index, bool &status, string &data) {
  ...
  if (line[index] == 'C') {
    ...
    data += identifier;
    while (index < line.length() &&
           line[index] != identifier) { // Copy all data until next identifier
    ...
    while (index < line.length() &&
           !checkWhiteSpace(line[index])) { // In no whitespace then data
      data += line[index];
      index++;
    }
  }
...
char EvaluateString::get() { return storedData[index++]; }

char EvaluateString::peek() { return storedData[index]; }
